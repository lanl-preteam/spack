...
def push_or_raise(spec: Spec, mirror: spack.mirror.Mirror, options: PushOptions):
    """
    ...
    with tempfile.TemporaryDirectory(dir=spack.stage.get_stage_root()) as tmpdir:
        _build_tarball_in_stage_dir(spec, mirror, stage_dir=tmpdir, options=options)


def _build_tarball_in_stage_dir(spec: Spec, mirror: spack.mirror.Mirror, stage_dir: str, options: PushOptions):
    ...
    fetch_spackfile_path = url_util.join(
        fetchdir, os.path.relpath(spackfile_path, stage_dir))
    push_spackfile_path = url_util.join(
        pushdir, os.path.relpath(spackfile_path, stage_dir))
    ...
    if web_util.url_exists(fetch_spackfile_path) and not options.force:
    ...
    push_specfile_path = url_util.join(
        pushdir, os.path.relpath(specfile_path, os.path.realpath(stage_dir)))
    push_signed_specfile_path = "{0}.sig".format(push_specfile_path)
    fetch_specfile_path = url_util.join(
        fetchdir, os.path.relpath(specfile_path, os.path.realpath(stage_dir)))
    fetch_signed_specfile_path = "{0}.sig".format(fetch_specfile_path)
    ...
    if options.force:
        ...
        if web_util.url_exists(push_signed_specfile_path):
            web_util.remove_url(push_signed_specfile_path)
    elif web_util.url_exists(push_specfile_path) or \
        web_util.url_exists(push_signed_specfile_path) or \
        web_util.url_exists(fetch_specfile_path) or \
        web_util.url_exists(fetch_signed_specfile_path) \
        :
    ...
    web_util.push_to_url(
        ...
        push_signed_specfile_path if not options.unsigned else push_specfile_path,
        keep_original=False,
    )
    ...
    # imported
    if not options.unsigned:
        push_keys(mirror.push_url, keys=[key], regenerate_index=options.regenerate_index, tmpdir=stage_dir)

    # create an index.json for the build_cache directory so specs can be
    # found
    if options.regenerate_index:
        generate_package_index(url_util.join(mirror.push_url, os.path.relpath(cache_prefix, stage_dir)))
...
def push(spec: Spec, mirror: spack.mirror.Mirror, options: PushOptions):
    ...
    """
    try:
        push_or_raise(spec, mirror, options)
